...
static hashing_mode_t hash_mode = HASH_MODE_MD5;

void ldb_hash_mode_select(int key_ln)
{
    if (key_ln == 8)
        hash_mode = HASH_MODE_CRC64;
    else
        hash_mode = HASH_MODE_MD5;
}
...
uint64_t crc64_ecma(const void *data, size_t length) {
    uint64_t crc = 0xFFFFFFFFFFFFFFFFULL;
    const uint8_t *p = (const uint8_t *)data;
    
    while (length--) {
        uint8_t table_index = (uint8_t)(crc ^ *p++);
        crc = crc64_ecma_table[table_index] ^ (crc >> 8);
    }
...
void md5_string(const unsigned char *input, int len, unsigned char output[16])
{
    ...
    if (hash_mode == HASH_MODE_CRC64)
    {
        uint64_t result = crc64_ecma(input, len);
        memset(output, 0, MD5_LEN);
        memcpy(output, &result, sizeof(uint64_t));
        return;
    }
    gcry_md_hd_t h;
